...
class TransitionSemanticDependencyModel(nn.Module):
    ...
    def dynamic_loss5(self, words, words_len, feats, edges, labels, p):
        ...
        while (unfinish_mask.sum()):
            ...
            if (self.loss_type == 'Formal'):
                ...
                this_step_action_embed = self.transition_embed(pred_action)
                # [k, n_transition_embed]
                label_final_repr = torch.cat(
                    (final_repr, this_step_action_embed), -1)
                label_score = self.label_mlp(label_final_repr)
                ...
                step_action_loss = self.do_criterion(action_score,
                                                     correct_actions)
                v_x = correct_mask.nonzero()[:, 0]
                v_y = new_correct_labels[correct_mask]
                label_target = torch.zeros((k, self.n_labels + 1),
                                           dtype=torch.long,
                                           device=words.device)
    ...
    def dynamic_loss6(self, words, words_len, feats, edges, labels, p):
        r"""
        目前只适用dual
        目前的优化目标只是Correct(c)中分数最高的
        words: [batch_size, seq_len]
        ...
        edges : [batch_size, seq_len, seq_len]
        labels: [batch_size, seq_len, seq_len]
        与4不同的是：交换了判断的顺序，和以前人工作一致
        """

        num_step = 0
        num_step_follow_false = 0
        ...
        while (unfinish_mask.sum()):
            ...
            pred_action_temp = pred_action.unsqueeze(1)
            mask_if_pred_correct = (
                ~(torch.gather(correct_mask, 1, pred_action_temp))).squeeze(
                    1)  # [k],false代表pred是正确的,true代表是错误的

            pro = torch.rand(k, device=words.device)
            # [k] 对应k个状态follow pred的概率,小于p则follow pred的，否则则follow o_action
            follow_o = pro.gt(p)
            if_change_mask = mask_if_pred_correct & follow_o

            num_step += k
            num_step_follow_false += (torch.sum(~if_change_mask) -
                                      torch.sum(~mask_if_pred_correct)).item()
            # num_follow_pred_step += torch.sum(follow_pred).item()

            followed_action = pred_action.masked_scatter(
                if_change_mask, o_action[if_change_mask])
            # [k] k个state真正要执行的action的下标
        ...
        losses = torch.stack(losses) * torch.tensor(remain_ks,
                                                    device=words.device)
        loss = torch.sum(losses) / len(losses)
        return loss, num_step, num_step_follow_false

    def dynamic_loss7(self, words, words_len, feats, edges, labels, p):
        r"""
        目前只适用dual
        目前的优化目标只是Correct(c)中分数最高的
        words: [batch_size, seq_len]
        ...
        while (unfinish_mask.sum()):
            ...
            action_score = self.action_mlp2(
                self.activate(self.action_mlp1(final_repr)))
            # [k, n_transitions]
            action_pro = nn.functional.softmax(
                nn.functional.softmax(action_score, 1) + 0.75)  # 超参0.75
            sampled_action = torch.multinomial(action_pro, 1).squeeze(1)  #[k]
