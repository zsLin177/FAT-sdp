...
class TransitionSemanticDependencyModel(nn.Module):
    ...
    def __init__(self,
        ...
        if (decode_mode == 'lstm'):
            ...
            # the MLP layer, do not splite the action and label
            self.activate = nn.ReLU()
            self.base_mlp1 = MLP(n_in=n_lstm_hidden * 2 * 3 +
                                 n_transition_embed,
                                 n_out=n_lstm_hidden,
                                 dropout=mlp_dropout,
                                 activation=False)

            self.base_mlp2 = MLP(n_in=n_lstm_hidden,
                                 n_out=n_transitions,
                                 dropout=mlp_dropout,
                                 activation=False)
            ...
            self.activate = nn.ReLU()
            self.action_mlp1 = MLP(n_in=n_lstm_hidden * 2 * 3 +
                                   n_transition_embed,
                                   n_out=n_lstm_hidden,
                                   dropout=mlp_dropout,
                                   activation=False)
            self.action_mlp2 = MLP(n_in=n_lstm_hidden,
                                   n_out=n_transitions,
                                   dropout=mlp_dropout,
                                   activation=False)

            self.label_mlp1 = MLP(n_in=n_lstm_hidden * 2 * 3 +
                                  n_transition_embed * 2,
                                  n_out=n_lstm_hidden,
                                  dropout=mlp_dropout,
                                  activation=False)
            self.label_mlp2 = MLP(n_in=n_lstm_hidden,
                                  n_out=n_labels + 1,
                                  dropout=mlp_dropout,
                                  activation=False)

        elif (decode_mode == 'par-dual'):
            self.activate = nn.ReLU()
            self.action_mlp1 = MLP(n_in=n_lstm_hidden * 2 * 3 +
                                   n_transition_embed,
                                   n_out=n_lstm_hidden,
        ...
        # pdb.set_trace()
        if(decode_mode == 'mlp'):
            self.build_label_action_map()

    def build_label_action_map(self):
        self.id2action = ["LR", "LP", "RS", "RP"]  #仅仅包含这四个带label的
        self.action2id = {}
        # pdb.set_trace()
        for idx, key in enumerate(self.id2action):
            self.action2id[key] = idx
        label_action_map = [[-1] * len(self.id2action)
                            for i in range(self.n_labels)]
        for idx, key in enumerate(self.transition_vocab.itos):
            real_action = key.split(':')[0]
            if (real_action in self.id2action):
                label_id = int(key.split(':')[1])
                real_action_id = self.action2id[real_action]
                label_action_map[label_id][real_action_id] = idx

        self.label_action_map = torch.tensor(label_action_map,
                                             dtype=torch.long)
    ...
    def forward(self, words, feats, states, gold_actions, transition_len):
        ...
        if (self.decode_mode == 'mlp'):
            score = self.base_mlp2(self.activate(self.base_mlp1(final_repr)))
            ...
            action_score = self.action_mlp2(
                self.activate(self.action_mlp1(final_repr)))
            # [batch_size, pad_len, n_transitions]
            ...
            label_score = self.label_mlp2(
                self.activate(self.label_mlp1(label_final)))
            return action_score, label_score

        elif (self.decode_mode == 'par-dual'):
            action_score = self.action_mlp2(
                self.activate(self.action_mlp1(final_repr)))
            # [batch_size, pad_len, n_transitions]
            # gold_mask = gold_actions.eq(-1)
            ...
            # label_final = torch.cat((final_repr, ga_embed), -1)
            label_score = self.label_mlp2(
                self.activate(self.label_mlp1(final_repr)))
            return action_score, label_score
        ...
        for i in range(batch_size):
            ...
            while (buffer_len > 0):
                ...
                if len(valid_actions) > 1:
                    ...
                    if (self.decode_mode == 'mlp'):
                        score = self.base_mlp2(
                            self.activate(self.base_mlp1(final_repr))).squeeze(
                                0)[torch.tensor(valid_actions,
                                                dtype=torch.long,
                                                device=x.device)]
                    elif (self.decode_mode == 'att'):
                        ...
                        score = self.action_mlp2(
                            self.activate(self.action_mlp1(
                                final_repr))).squeeze(0)[torch.tensor(
                                    valid_actions,
                                    dtype=torch.long,
                                    device=x.device)]
                ...
                if action in action_id["LR"] or action in action_id["LP"] or \
                    ...
                    if (self.decode_mode != 'dual'):
                        ...
                        label_score = self.label_mlp2(
                            self.activate(self.label_mlp1(
                                label_final))).squeeze(0)[torch.tensor(
                                    [vla for vla in range(self.n_labels)],
                                    dtype=torch.long,
                                    device=x.device)]
    ...
    def dynamic_loss4(self, words, words_len, feats, edges, labels, p):
        ...
        while (unfinish_mask.sum()):
            ...
            valid1 = torch.ones((k, len(self.transition_vocab)),
                                dtype=torch.long,
                                device=words.device)
            base_mask = valid1.gt(0)  # 可以去掉？
            valid1_mask = base_mask.clone()  # 可以去掉？
            stat_len = remain_stat.gt(-1).sum(dim=2)
            # [k, 4]
            valid_mask_sg1 = stat_len[:, 0].gt(1)
            # [k]
            ...
            label_final_repr = torch.cat((final_repr, this_step_action_embed),
                                         -1)
            label_score = self.label_mlp2(
                self.activate(self.label_mlp1(label_final_repr)))
            ...
            pro = torch.rand(k, device=words.device)
            # [k] 对应k个状态follow pred的概率,小于p则follow pred的，否则则follow o_action
            follow_pred = pro.lt(p)

            # num_follow_pred_step += torch.sum(follow_pred).item()
            ...
            remain_stat[:, 3, 0] = followed_action
            # 到此remain _stat修改完毕,接下来把状态写回save_stat
            state_back_mask = unfinish_mask.unsqueeze(1).expand(
                -1, 4).unsqueeze(2).expand(-1, -1, seq_len)
            save_stat = save_stat.masked_scatter(state_back_mask, remain_stat)
            unfinish_mask = save_stat[:, 1, 0].gt(-1)
    ...
    def batch_decode(self, words, words_len, feats):
        ...
        col_cond4 = torch.zeros(len(self.transition_vocab),
                                device=words.device).index_fill_(
                                    ...
                                    torch.tensor(action_id['NR'] +
                                                 action_id['NP'],
                                                 device=words.device), 1).gt(0)
        reduce_idx = torch.tensor(action_id['LR'] + action_id['NR'],
                                  dtype=torch.long,
                                  device=words.device)
        pass_idx = torch.tensor(action_id['LP'] + action_id['NP'] +
                                action_id['RP'],
                                dtype=torch.long,
                                device=words.device)
        shift_idx = torch.tensor(action_id['RS'] + action_id['NS'],
                                 dtype=torch.long,
                                 device=words.device)
        ...
        labels = -torch.ones((batch_size, seq_len, seq_len),
                             dtype=torch.long,
                             device=words.device)
        left_idx = torch.tensor(action_id['LR'] + action_id['LP'],
                                dtype=torch.long,
                                device=words.device)
        right_idx = torch.tensor(action_id['RS'] + action_id['RP'],
                                 dtype=torch.long,
                                 device=words.device)
        ...
        while (unfinish_mask.sum()):
            ...
            require_left_mask = pred_action.unsqueeze(-1).expand(
                -1, left_idx.shape[0]).eq(left_idx).any(dim=1)
            require_right_mask = pred_action.unsqueeze(-1).expand(
                -1, right_idx.shape[0]).eq(right_idx).any(dim=1)
            ...
            #     action_id['RS'][0]) + followed_action.eq(action_id['NS'][0])
            # [k]
            Reduce_mask = followed_action.unsqueeze(-1).expand(
                -1, reduce_idx.shape[0]).eq(reduce_idx).any(dim=1)
            Pass_mask = followed_action.unsqueeze(-1).expand(
                -1, pass_idx.shape[0]).eq(pass_idx).any(dim=1)
            Shift_mask = followed_action.unsqueeze(-1).expand(
                -1, shift_idx.shape[0]).eq(shift_idx).any(dim=1)
    ...
    def batch_decode_2(self, words, words_len, feats):
        'label不依赖于action,and dual'
        batch_size, seq_len = words.shape
        # get the mask and lengths of given batch
        mask = words.ne(self.pad_index)
        ext_words = words
    ...
    def batch_decode_3(self, words, words_len, feats):
        # pdb.set_trace()
        # for decoder mode='mlp'
        batch_size, seq_len = words.shape
        # get the mask and lengths of given batch
        mask = words.ne(self.pad_index)
        ext_words = words
        ...
        unfinish_mask = save_stat[:, 1, 0].gt(-1)
        # pdb.set_trace()
        temp_keys = list(self.transition_vocab.stoi.keys())
        for key in temp_keys:
            if (key not in self.transition_vocab.itos):
                del self.transition_vocab.stoi[key]
                # print('del ' + key)
        ...
        # pdb.set_trace()
        label_id = {}
        # compose_action_id: label_id`
        # pdb.set_trace()
        for key in ["LR", "LP", "RS", "RP", "NS", "NR", "NP"]:
            lst_t = action_id[key]
            for idx in lst_t:
                sp_lst = self.transition_vocab.itos[idx].split(':')
                if (len(sp_lst) > 1):
                    label_id[idx] = int(
                        self.transition_vocab.itos[idx].split(':')[1])
                else:
                    label_id[idx] = -1
        left_idx = torch.tensor(action_id['LR'] + action_id['LP'],
                                dtype=torch.long,
                                device=words.device)
        right_idx = torch.tensor(action_id['RP'] + action_id['RS'],
                                 dtype=torch.long,
                                 device=words.device)
        col_cond1 = torch.zeros(
            len(self.transition_vocab), device=words.device).index_fill_(
        ...
        while (unfinish_mask.sum()):
            ...
            # action_score = self.action_mlp(final_repr)
            action_score = self.base_mlp2(
                self.activate(self.base_mlp1(final_repr)))

            # [k, n_transitions]
            new_v = torch.tensor([-float('inf')] * self.n_transitions,
                                 device=words.device).unsqueeze(0).expand(
                                     k, -1)
            new_v = new_v.masked_scatter(valid_mask, action_score[valid_mask])
            pred_action = torch.argmax(new_v, dim=1)
            ...
            pred_label = torch.tensor([
                label_id[pred_action_id]
                for pred_action_id in pred_action.tolist()
            ],
                                      dtype=torch.long,
                                      device=words.device)
            require_left_mask = pred_action.unsqueeze(-1).expand(
    ...
    def dynamic_loss4_single(self, words, words_len, feats, edges, labels, p):
        """
        different from loss4, it is for single stage model
        """
        batch_size, seq_len = words.shape
        # get the mask and lengths of given batch
        mask = words.ne(self.pad_index)
        ...
        while (unfinish_mask.sum()):
            ...
            pred_action = torch.argmax(new_v, dim=1)
            # [k] 存放的是预测的action的下标,由于pred_label并不会对state造成啥影响，所以暂时就没算
            new_c = torch.tensor([-float('inf')] * self.n_transitions,
                                 device=words.device).unsqueeze(0).expand(
                                     k, -1)
            new_c = new_c.masked_scatter(correct_mask,
                                         action_score[correct_mask])
            ...
            # 计算这一步的loss
            step_loss = self.criterion(action_score, o_action)
            # step_action_loss = self.criterion(action_score, pred_action)  # 让优化目标就是自己
            # this_step_action_embed = self.transition_embed(pred_action)
            # # [k, n_transition_embed]
            # label_final_repr = torch.cat((final_repr, this_step_action_embed),
    ...
    def dynamic_loss4_single_fix(self, words, words_len, feats, edges, labels,
                                 p):
        """
        different from loss4, it is for single stage model
        different from loss4 single, fix the correct set
        """
        batch_size, seq_len = words.shape
        # get the mask and lengths of given batch
        mask = words.ne(self.pad_index)
        ...
        while (unfinish_mask.sum()):
            ...
            if (stack_isnot_clear.sum().item() > 0):
                ...
                # [m]  不存在向右的边 and i与beta之间没有边 ,TODO: 加上要存在向左的边这个条件
                f_left_label = left_label[final_lr]
                lr_real_id = self.action2id['LR']
                transition_id = self.label_action_map[f_left_label, lr_real_id].to(final_lr.device)
                transition_id_mask = transition_id.gt(-1)
                final_lr = final_lr.masked_scatter(
                    final_lr,
                    transition_id_mask)  # 也许存在有这个边，且可以lr但是没有lr：label这个组合
                if(final_lr.any()):
                    final_lr_1 = final_lr.unsqueeze(1).expand(
                        -1, len(self.transition_vocab))
                    # [m, n_actions]
                    transition_id = transition_id[transition_id_mask]
                    lr_action = torch.zeros(
                        (transition_id.shape[0], len(self.transition_vocab)),
                        dtype=torch.long,
                        device=words.device)
                    
                    lr_action[torch.arange(0,
                                        transition_id.shape[0],
                                        dtype=torch.long,
                                        device=words.device), transition_id] = 1
                    m_actions = m_actions.masked_scatter(final_lr_1, lr_action)
                ...
                f_right_label = right_label[final_rs]
                rs_real_id = self.action2id['RS']
                transition_id = self.label_action_map[f_right_label,
                                                    rs_real_id].to(final_rs.device)
                transition_id_mask = transition_id.gt(-1)
                final_rs = final_rs.masked_scatter(final_rs,
                                                transition_id_mask)
                if(final_rs.any()):
                    final_rs_1 = final_rs.unsqueeze(1).expand(
                        -1, len(self.transition_vocab))
                    transition_id = transition_id[transition_id_mask]
                    rs_action = torch.zeros(
                        (transition_id.shape[0], len(self.transition_vocab)),
                        dtype=torch.long,
                        device=words.device)
                    rs_action[torch.arange(0,
                                        transition_id.shape[0],
                                        dtype=torch.long,
                                        device=words.device), transition_id] = 1
                    m_actions = m_actions.masked_scatter(final_rs_1, rs_action)
                # col_rs = torch.zeros(len(self.transition_vocab),
                #                      device=words.device).index_fill_(
                #                          -1,
                #                          torch.tensor(action_id['RS'],
                ...
                final_lp = (~exist_right) & exist_left  # TODO: 加上要存在向左的边这个条件
                f_left_label = left_label[final_lp]
                lp_real_id = self.action2id['LP']
                transition_id = self.label_action_map[f_left_label, lp_real_id].to(final_lp.device)
                transition_id_mask = transition_id.gt(-1)
                final_lp = final_lp.masked_scatter(final_lp,
                                                transition_id_mask)
                if(final_lp.any()):
                    final_lp_1 = final_lp.unsqueeze(1).expand(
                        -1, len(self.transition_vocab))
                    transition_id = transition_id[transition_id_mask]
                    lp_action = torch.zeros(
                        (transition_id.shape[0], len(self.transition_vocab)),
                        dtype=torch.long,
                        device=words.device)
                    lp_action[torch.arange(0,
                                        transition_id.shape[0],
                                        dtype=torch.long,
                                        device=words.device), transition_id] = 1
                    m_actions = m_actions.masked_scatter(final_lp_1, lp_action)
                ...
                final_rp = (~exist_left) & exist_right  # TODO: 加上要存在向右的边这个条件
                f_right_label = right_label[final_rp]
                rp_real_id = self.action2id['RP']
                transition_id = self.label_action_map[f_right_label,
                                                    rp_real_id].to(final_rp.device)
                transition_id_mask = transition_id.gt(-1)
                final_rp = final_rp.masked_scatter(final_rp,
                                                transition_id_mask)
                if(final_rp.any()):
                    final_rp_1 = final_rp.unsqueeze(1).expand(
                        -1, len(self.transition_vocab))
                    transition_id = transition_id[transition_id_mask]
                    rp_action = torch.zeros(
                        (transition_id.shape[0], len(self.transition_vocab)),
                        dtype=torch.long,
                        device=words.device)
                    rp_action[torch.arange(0,
                                        transition_id.shape[0],
                                        dtype=torch.long,
                                        device=words.device), transition_id] = 1
                    m_actions = m_actions.masked_scatter(final_rp_1, rp_action)
                # col_rp = torch.zeros(len(self.transition_vocab),
                #                      device=words.device).index_fill_(
                #                          -1,
                #                          torch.tensor(action_id['RP'],
